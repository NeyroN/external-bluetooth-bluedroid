...
void rtk_vendor_cmd_to_fw(uint16_t opcode, uint8_t parameter_len, uint8_t* parameter)
{
    ...
    if(bt_hc_cbacks)
	    p_buf = (HC_BT_HDR *) bt_hc_cbacks->alloc(BT_HC_HDR_SIZE + HCI_CMD_PREAMBLE_SIZE + parameter_len);
    ...
    p_hci_if->send_int_cmd(opcode, p_buf, NULL);
...
void rtk_notify_profileinfo_to_fw()
{
    ...
    if(bt_hc_cbacks)
	    p_buf = (uint8_t *) bt_hc_cbacks->alloc(buffer_size);
...
void udpsocket_recv(uint8_t *recv_msg, uint8_t *msg_size)
{
	...
	*msg_size = n;
    ...
    //int kk = 0;
    //for(kk; kk<n; kk++)
        //LogMsg("recv udp data %x", recv_msg[kk]);
}
